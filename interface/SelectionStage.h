#ifndef SELECTIONSTAGE_H
#define SELECTIONSTAGE_H

// Renamed from CutStage -> SelectionStage (agreed 2026-08-06, done as its
// own step - see PHASE1.md). Pure rename: file name, enum name,
// cutIndex() -> selectionIndex(), kNumCutStages -> kNumSelectionStages.
// No behavior change - same 10 stages, same values, same meaning.
...
enum class SelectionStage : std::size_t {
...
constexpr std::size_t kNumSelectionStages = static_cast<std::size_t>(SelectionStage::Count);

// Use this instead of a raw static_cast at every call site - e.g.
//   h_Lep1pt[selectionIndex(SelectionStage::AfterZVeto)]->Fill(...)
// instead of
//   h_Lep1pt[2]->Fill(...)
constexpr std::size_t selectionIndex(SelectionStage stage) {
    return static_cast<std::size_t>(stage);
}

#endif  // SELECTIONSTAGE_H
